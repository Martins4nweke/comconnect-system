...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    ...
    return fail(error?.message ?? "Failed to load referral follow-ups", 500);
  }
}
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    if (!canManageReferrals(context)) {
      return fail("You do not have permission to create referral follow-ups.", 403);
    }
    ...
    return fail(error?.message ?? "Failed to create referral follow-up", 400);
  }
}