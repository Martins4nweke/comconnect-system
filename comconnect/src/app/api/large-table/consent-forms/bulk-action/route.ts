...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!canManageConsent(context)) {
      return fail("You do not have permission to update consent forms.", 403);
    }
    ...
    if (allowedIds.length === 0) {
      return fail("No selected consent forms are accessible.", 403);
    }
    ...
    if (action === "status") {
      ...
      if (!allowedStatuses.has(status)) {
        return fail("Invalid consent form status.", 400);
      }
    ...
    return fail(error?.message ?? "Bulk consent form action failed", 500);
  }
}