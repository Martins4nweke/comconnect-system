...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!canManageSchedules(context)) {
      return fail("You do not have permission to update schedules.", 403);
    }
    ...
    if (allowedIds.length === 0) {
      return fail("No selected schedules are accessible.", 403);
    }
    ...
    if (action === "status") {
      ...
      if (!allowedStatuses.has(status)) {
        return fail("Invalid schedule status.", 400);
      }
    ...
    return fail(error?.message ?? "Bulk scheduler action failed", 500);
  }
}