...
export async function POST(req: NextRequest) {
  try {
    ...
    if (allowedIds.length === 0) {
      return fail("No selected questionnaires are accessible.", 403);
    }
    ...
    if (action === "status") {
      ...
      if (!allowedStatuses.has(status)) {
        return fail("Invalid questionnaire status.", 400);
      }
    ...
    return fail(error?.message ?? "Bulk questionnaire action failed", 500);
  }
}