...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!canManagePushQueue(context)) {
      return fail("You do not have permission to update push queue items.", 403);
    }
    ...
    if (allowedIds.length === 0) {
      return fail("No selected push queue items are accessible.", 403);
    }
    ...
    if (action === "status") {
      ...
      if (!allowedStatuses.has(status)) {
        return fail("Invalid push queue status.", 400);
      }
    ...
    return fail(error?.message ?? "Bulk push queue action failed", 500);
  }
}