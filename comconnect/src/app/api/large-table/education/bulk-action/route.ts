...
export async function POST(req: NextRequest) {
  try {
    ...
    if (allowedIds.length === 0) {
      return fail("No selected education items are accessible.", 403);
    }
    ...
    if (action === "status") {
      ...
      if (!allowedStatuses.has(status)) {
        return fail("Invalid education status.", 400);
      }
    ...
    return fail(error?.message ?? "Bulk education action failed", 500);
  }
}