...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!canManageAppointments(context)) {
      return fail("You do not have permission to update appointments.", 403);
    }
    ...
    if (allowedIds.length === 0) {
      return fail("No selected appointments are accessible.", 403);
    }
    ...
    if (action === "status") {
      ...
      if (!allowedStatuses.has(status)) {
        return fail("Invalid appointment status.", 400);
      }
    ...
    return fail(error?.message ?? "Bulk appointment action failed", 500);
  }
}