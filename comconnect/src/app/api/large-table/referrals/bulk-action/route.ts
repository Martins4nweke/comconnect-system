...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!canManageReferrals(context)) {
      return fail("You do not have permission to update referrals.", 403);
    }
    ...
    if (allowedIds.length === 0) {
      return fail("No selected referrals are accessible.", 403);
    }
    ...
    if (action === "status") {
      ...
      if (!allowedStatuses.has(status)) {
        return fail("Invalid referral status.", 400);
      }
    ...
    if (action === "follow_up") {
      const followUpAt = cleanText(body?.follow_up_at);

      if (!followUpAt) {
        return fail("follow_up_at is required", 400);
      }
    ...
    return fail(error?.message ?? "Bulk referral action failed", 500);
  }
}