...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!canManageChat(context)) {
      return fail("You do not have permission to update chat threads.", 403);
    }
    ...
    if (allowedIds.length === 0) {
      return fail("No selected chat threads are accessible.", 403);
    }
    ...
    if (action === "status") {
      ...
      if (!allowedStatuses.has(status)) {
        return fail("Invalid chat thread status.", 400);
      }
    ...
    return fail(error?.message ?? "Bulk chat action failed", 500);
  }
}